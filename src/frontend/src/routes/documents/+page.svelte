<script>
    ...
    import { TrashCan, CheckmarkFilled, StringInteger, StringText, CalendarHeatMap, Term, Parameter} from "carbon-icons-svelte";
...
<DataTable
    ...
    description="A collection of your organization's active schemas used for extracting structured data from documents. Use the dropdown menu to view the schema details."
    batchExpansion
    batchSelection
    bind:selectedRowIds
    {headers}
    ...
    <svelte:fragment slot="cell" let:cell>
        {#if cell.key === "schema_definition"}
            {#each Object.entries(cell.value.properties) as [field_name, field_object]}
                ...
                <Tag icon={icon} type={color}>{field_name}</Tag>
            {/each}
